...
def test_build_prompt_loads_and_renders():
    ...
    assert "platform" not in prompt or "{{platform}}" not in prompt
    assert "X" in prompt
    assert "10代女性" in prompt


# ---- Gemini API 呼び出し (google.genai.Client をモック) ----
...
def test_generate_json_missing_api_key_returns_none(monkeypatch):
    monkeypatch.setattr("src.ai.gemini_client.settings.gemini_api_key", "")
    result = generate_json("test prompt")
    assert result is None


def test_generate_json_uses_sdk_parsed_when_available(monkeypatch):
    monkeypatch.setattr("src.ai.gemini_client.settings.gemini_api_key", "dummy-key")
    monkeypatch.setattr("src.ai.gemini_client.time.sleep", lambda *_: None)

    parsed_instance = DummySchema(score=90, reason="良い")
    response = MagicMock(parsed=parsed_instance, text='{"score": 90, "reason": "良い"}')
    mock_client = _make_mock_client(response)
    monkeypatch.setattr(
        "src.ai.gemini_client.genai.Client", lambda api_key: mock_client
    )
...
def test_generate_json_falls_back_to_manual_parse_when_parsed_is_none(monkeypatch):
    monkeypatch.setattr("src.ai.gemini_client.settings.gemini_api_key", "dummy-key")
    monkeypatch.setattr("src.ai.gemini_client.time.sleep", lambda *_: None)

    response = MagicMock(parsed=None, text='{"score": 75, "reason": "まずまず"}')
    mock_client = _make_mock_client(response)
    monkeypatch.setattr(
        "src.ai.gemini_client.genai.Client", lambda api_key: mock_client
    )
...
def test_generate_json_without_schema_returns_dict(monkeypatch):
    ...
    response = MagicMock(parsed=None, text='{"foo": "bar"}')
    mock_client = _make_mock_client(response)
    monkeypatch.setattr(
        "src.ai.gemini_client.genai.Client", lambda api_key: mock_client
    )
...
def test_generate_json_invalid_json_returns_none(monkeypatch):
    ...
    response = MagicMock(parsed=None, text="これはJSONではありません")
    mock_client = _make_mock_client(response)
    monkeypatch.setattr(
        "src.ai.gemini_client.genai.Client", lambda api_key: mock_client
    )
...
def test_generate_json_schema_mismatch_returns_none(monkeypatch):
    ...
    response = MagicMock(parsed=None, text='{"reason": "理由のみ"}')
    mock_client = _make_mock_client(response)
    monkeypatch.setattr(
        "src.ai.gemini_client.genai.Client", lambda api_key: mock_client
    )
...
def test_generate_json_retries_then_succeeds(monkeypatch):
    ...
    success_response = MagicMock(parsed=None, text='{"foo": "bar"}')
    mock_client = MagicMock()
    mock_client.models.generate_content.side_effect = [
        APIError(code=503, response_json={"error": "unavailable"}),
        success_response,
    ]
    monkeypatch.setattr(
        "src.ai.gemini_client.genai.Client", lambda api_key: mock_client
